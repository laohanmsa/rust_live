...
市场资料来自 Django 内网 `/api/trading-context/`，启动时完整分页读取，之后每 30 秒核对；新提案会触发批量补取。
原型不修改原生产的订阅注册、市场、订单或资金。
...
当前 M5 估值从 Django 最近五分钟且同一请求的结果中读取，缺失就跳过；这不证明旧提案到估值的等待已经消失。
新提案的上下文/估值晚到会单独记为等待或拒绝，模拟接受率不是实盘成单率。
...
`./observe.sh --json` 的 `sources` 包含连接、同步代次、候选数量、待补资料、生命周期事件计数、时间戳类型和最近模拟订单。
